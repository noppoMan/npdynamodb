...
function Migrator(dynamodb){
  var apiVer = dynamodb.config.apiVersion;
...
Migrator.prototype.createTable = function(tableName, callback){
  ...
  return this.npd().table(tableName).createTable(builder);
}
...
MigrateRunner.prototype.run = function(){
  ...
  .then(function(){
    ...
    .then(function(data){
      ...
      var dirs = fs.readdirSync(self.config.migrationRoot);
      ...
      dirs.forEach(function(dir){
        ...
        if(!isFound) {
          var migratorFile = require(self.config.migrationRoot+'/'+dir);
          ...
          tasks.push(
            new Promise(function(resolve, reject){
              ...
              .then(function(){
                console.log('Migrated ' + self.config.migrationRoot+'/'+dir);
                resolve();
              })
              .catch(reject);
            })
...
MigrateRunner.prototype.rollback = function(){
  ...
  .then(function(data){
    ...
    return pglob(path.join(self.config.migrationRoot, '/' + lastVersion + "_*.js"))
