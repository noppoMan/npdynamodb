...
var npdybamodb = require('../npdynamodb');
...
module.exports = function(tableName, params, customProps){
  ...
  var ormProxy = {
    ...
    npdynamodb: function(){
      return npdybamodb(params.dynamodb);
    },
  ...
  if(customProps) {
    _.map(customProps, function(value, name){
      if(value.hasOwnProperty('bind')) {
        ormProxy[name] = value.bind(ormProxy);
      }else{
        ormProxy[name] = value;
      }
    });
...
function Orm(proxy, customProps){
  var self = this;
  ...
  this.npdynamodb = function(){ return npdybamodb(proxy.config.dynamodb); };

  if(customProps) {
    _.each(customProps, function(value, name){
      if(value.hasOwnProperty('bind')) {
...
Orm.prototype.save = function(item){
  ...
  return promiseRunner(this._builder.create(this.attributes()), function(data){
    return new Orm({
      config: self.config,
